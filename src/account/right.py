...
from models.right import Rights, Rights_Pydantic, RoleRight, Roles, Roles_Pydantic
...
@router.get("/account/getRoleList", tags=["account"], responses={404: {"model": HTTPNotFoundError}})
...
@router.get("/account/getRightsByRoleId", tags=["account"], responses={404: {"model": HTTPNotFoundError}})
async def get_rights_by_role_id(id: int):
    rights = await RoleRight.filter(role=id).prefetch_related('right').all()
    return {
        "msg": "ok",
        "data": [{
            "id": right.right.id,
...
async def update_role(body: RoleModel):
    # 删除多余的权限
    await RoleRight.filter(role=body.id).filter(right_id__not_in=body.right_list).delete()
    # 获取已创建的权限
    had_right_map = await RoleRight.filter(role=body.id).filter(right_id__in=body.right_list).values('right_id')
